...
for dm in [0]:
    ...
    for L in tqdm(sizes1):
        ...
        if T_S_plot:
            ...
            for ii in range(len(dt_ave)):
                ...
                plt.xlabel("time")
                plt.ylabel(r'E[S]')
                plt.title(f'Mutual Information vs. Time {dt8[1][ii][-1]}D, Num. of Iters={len(dt8[0][ii])}, MP={str(MP[ii])}')
                plt.savefig(f"plots/MIsT=({dt8[1][0][1]},{dt8[1][0][2]})_L={L}_Iters={len(dt8[0][ii])}_MP={MP[ii]}_{dt8[1][ii][-1]}D.png")
                plt.show()
                plt.close()
    ...
    if P_S_plot:
        ...
        filename   = f"plots/MPvsIMI_Ave_L={sizes1}_T=({dt8[1][0][1]},{dt8[1][0][2]})_Ballistic_{dt8[1][0][-1]}D.png"
        ylabel     = "E[p(MP)]"
        plot_mp_vs_quant(Data1, sizes1, plot_title, filename, ylabel=ylabel)
    
        Dt = invert_data_structure(Data1.copy(), sizes1)
        
        plot_title = f"System Size vs. Averaged Mutual Information\n{dt8[1][0][-1]}D - Ballistic Regime"
        filename   = f"plots/LvsMI_Ave_L={sizes1}_T=({dt8[1][0][1]},{dt8[1][0][2]})_Ballistic_{dt8[1][0][-1]}D.png"
        xlabel     = "L"
        ylabel     = "E[p(MP)]"
        ...
        plot_title = f"Measurement Probability vs. Log. Variations of Averaged Mutual Information\n{dt8[1][0][-1]}D - Ballistic Regime"
        filename   = f"plots/MPvsVar_MI_Ave(dE[p(MP))\dL)_L={sizes1}_T=({dt8[1][0][1]},{dt8[1][0][2]})_Ballistic_{dt8[1][0][-1]}D.png"
        ylabel     = "dp(MP)/dL"
        plot_mp_vs_var_quant(Data1, sizes1, plot_title, filename, ylabel=ylabel, func=perimeter)

        # Quadratic Case:
        area = lambda xx: xx*xx/4
        plot_title = f"Measurement Probability vs. Linear Variations of Averaged Mutual Information\n{dt8[1][0][-1]}D - Ballistic Regime"
        filename   = f"plots/MPvsVar_MI_Ave(dE[p(MP)])\dL)_L={sizes1}_T=({dt8[1][0][1]},{dt8[1][0][2]})_Ballistic_{dt8[1][0][-1]}D.png"
        ylabel     = "dp(MP)/dL**2"
