...
pmax = 0.55
pstep = 0.05
ps = list(np.arange(0.25, pmax, pstep))
        ...
        for L in Ls:
            ...
            for Prob in ps:
                ...
                with mp.Pool(processes=N_PROCESSES) as pool:
                    ...
                    if all(k in to_calc for k in quants): 
                        async_result = pool.map_async(run_one_simulation_general, args_list)
                    if len(to_calc)==1 and "EEnt" in to_calc: 
                        async_result = pool.map_async(run_one_simulation_EEnt, args_list)
                    if len(to_calc)==1 and "IPR"  in to_calc: 
                        async_result = pool.map_async(run_one_simulation_IPR, args_list)
                    if len(to_calc)==1 and "MI"  in to_calc: 
                        async_result = pool.map_async(run_one_simulation_MI, args_list)
                ...
                if "EEnt" in to_calc:
                    out_file_EEnt = (
                        f"{dir_name}/EEqq_{L}_p={round(Prob, 2)}_T=({Time},{t_step},{Tmode})_EntEntr={EntEntr}"
                    )
                    if ballistic: out_file_EEnt += "_Ballistic"

                    if accumulate:
                        with open(out_file_EEnt + "_1D", "a") as f:
                            np.savetxt(f,out_arr_EEnt) 
                    if not accumulate: np.savetxt(out_file_EEnt + "_1D", out_arr_EEnt)
                    
                    print(f"saved -> {out_file_EEnt}")

                if "IPR" in to_calc:
                    out_file_IPR = (
                        f"{dir_name}/IPR_{L}_p={round(Prob, 2)}_T=({Time},{t_step},{Tmode})_amp={amp}"
                    )
                    if ballistic: out_file_IPR  += "_Ballistic"

                    if accumulate:
                        with open(out_file_IPR + "_1D", "a") as f:
                            np.savetxt(f,out_arr_IPR) 
                    if not accumulate: np.savetxt(out_file_IPR  + "_1D", out_arr_IPR)

                    print(f"saved -> {out_file_IPR}")
                
                if "MI" in to_calc:
                    out_file_MI = (
                        f"{dir_name}/MI_{L}_p={round(Prob, 2)}_T=({Time},{t_step},{Tmode})"
                    )
                    if ballistic: out_file_MI  += "_Ballistic"

                    if accumulate:
                        with open(out_file_MI + "_1D", "a") as f:
                            np.savetxt(f,out_arr_MI) 
                    if not accumulate: np.savetxt(out_file_MI  + "_1D", out_arr_MI)
