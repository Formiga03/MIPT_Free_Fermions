    ...
    with mp.Manager() as manager:
        ...
        for L in Ls:
            ...
            for Prob in ps:
                ...
                out_file = (
                    f"{dir_name}/IPR_{L}_p={round(Prob, 2)}_T=({Time},{t_step})"
                )
                if ballistic:
                    out_file += "_Ballistic"
...
def main():
    ...
    for L in Ls:
        ...
        for Prob in ps:
            ...
            # Prepare list of arguments, one per trajectory
            args_list = [
                (round(Prob, 2), Time, Steps1, U0, C0, ballistic)
                for _ in range(N_TRAJ)
            ]
            ...
            with mp.Pool(processes=N_PROCESSES) as pool:
                ...
                results_list = list(tqdm(pool.imap(run_one_simulation_IPR, args_list),
                                         total=N_TRAJ))
            ...
            out_file = (
                f"{dir_name}/IPR_{L}_p={round(Prob, 2)}_T=({Time},{t_step})_1D"
            )
