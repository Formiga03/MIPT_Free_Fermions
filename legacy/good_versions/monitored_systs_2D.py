...
#Steps1 = list(np.arange(Time-15, Time, t_step))
...
def main():
    ...
    with mp.Manager() as manager:
        ...
        for L in Ls:
            ...
            U0, C0 = build_initial_conditions(L, "2D")
            ...
            for Prob in ps:
                ...
                out_file = (
                    f"{dir_name}/EEqq_{L*L}_p={round(Prob, 2)}_T=({Time},{t_step})_EntEntr={EntEntr}"
                )
                if ballistic:
                    out_file += "_Ballistic"
...
def main():
    ...
    for L in Ls:
        ...
        for Prob in ps:
            ...
            # Prepare list of arguments, one per trajectory
            args_list = [
                (round(Prob, 2), Time, Steps1, U0, C0, EntEntr, ballistic)
                for _ in range(N_TRAJ)
            ]
            ...
            out_file = f"{dir_name}/EEqq_{L*L}_p={round(Prob, 2)}_T=({Time},{t_step})_EntEntr={EntEntr}"
            if ballistic: out_file += "_Ballistic"
            np.savetxt(out_file + "_2D", out_arr)
            print(f"saved -> {out_file}")
"""
